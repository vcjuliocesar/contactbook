<template>
  <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
    ...
    <tbody>
      <tr v-for="(contact,index) in contacts" :key="contact.id" @keydown="clear($event.target.name)">
        <td>{{ contact.id }}</td>
        <td>
          <input
            v-if="id === contact.id"
        ...
        <td>
          ...
          <button v-else class="btn btn-warning btn-icon-split" @click="edit(contact)">
            <span class="icon text-white-50">
              <i class="fas fa-edit"></i>
            </span>
            <span class="text">Edit</span>
          </button>
          <button class="btn btn-danger btn-icon-split" @click="erase(index, contact)">
            <span class="icon text-white-50">
    ...
    methods: {
        ...
        erase(index, contact) {
            axios
                ...
                .finally(() => alert('usuario eliminado'));
        },
